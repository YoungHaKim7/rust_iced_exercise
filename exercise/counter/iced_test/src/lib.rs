...
use crate::core::{Element, Event, Font, Point, Rectangle, Settings, Size, SmolStr};
    ...
    Renderer: core::Renderer + core::renderer::Headless,
{
    ...
    pub fn with_size(
        ...
        let mut renderer = Renderer::new(default_font, settings.default_text_size);
    ...
    pub fn find(&mut self, selector: impl Into<Selector>) -> Result<Target, Error> {
        ...
        match &selector {
            Selector::Id(id) => {
                ...
                impl widget::Operation for FindById<'_> {
                    ...
                    fn text(&mut self, id: Option<&widget::Id>, bounds: Rectangle, _text: &str) {
                        if self.target.is_some() {
                            return;
                        }
    ...
    /// - Simulating a [`click`].
    pub fn click(&mut self, selector: impl Into<Selector>) -> Result<Target, Error> {
    ...
    pub fn snapshot(&mut self, theme: &Theme) -> Result<Snapshot, Error> {
        ...
        Ok(Snapshot {
            screenshot: window::Screenshot::new(rgba, physical_size, f64::from(scale_factor)),
        })
    }
